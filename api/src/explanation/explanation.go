...
Give a brief explanation: the translation, plus grammar notes only when they clarify this specific word in this specific sentence, such as a noun's gender/case, a verb form, or an idiomatic usage. If the word appears in its plain, uninflected dictionary form and its usage is unremarkable, give only the translation - do NOT add a grammar note just to have one (e.g. never point out that an adjective is uninflected or that a word is in its base form). Be grammatically precise: name an inflected form fully and exactly once (e.g. "3rd person singular present"), never vaguely if it is contains any ambiguity ("a present-tense form"). Do not name the part of speech ("is a noun", "is a verb") as a standalone fact if it's obvious from the translation. Mention the part of speech only when it resolves a real ambiguity in this sentence.
...
Example: for the word "Fenster" in the sentence fragment "aus dem Fenster", a good explanation for a English speaker learning German is: `+bt+`"das Fenster" means "window". Here it is dative singular ("dem Fenster") because the preposition "aus" takes the dative; "aus dem Fenster" means "out of the window".`+bt+`
...
Counterexample: `+bt+`"Klein" means "small" in "Das Kino ist klein" - "the cinema is small". It appears as an uninflected predicate adjective after "ist".`+bt+` - the second sentence adds nothing: an uninflected base form needs no grammar note. Write only: `+bt+`"Klein" means "small" in "Das Kino ist klein" - "the cinema is small".`+bt+`
...
Counterexample: `+bt+`"Saal" means "auditorium" or "hall"; here, in the cinema context, "der dunkle Saal" is "the dark screening room". It is masculine nominative singular: "der Saal".`+bt+` - the last sentence is not needed, the gender can be infered from the article and nominative can be dropped by default: `+bt+`"der Saal" means "hall" here; in the cinema context, "der dunkle Saal" is "the dark screening room".`+bt+`

Counterexample: `+bt+`"das Kino" means "the cinema" or "the movie theater"; here it refers to the old cinema building.`+bt+` - the last sentence duplicates the information from the story without adding value to the explanation from the language learner's perspective. Write only: `+bt+`"das Kino" means "the cinema" or "the movie theater".`+bt+`

The rule of thumb: every output word must count. Majority of the explanations should be 20 words or less, ideally under 10.
